...
public class SQLiteLifeOsReviewRepository implements LifeOsReviewRepository {
    ...
    public List<RecentRecordItem> getTagDetailRecords(String scope, String tagName, String startDate, String endDate,
        ...
        if ("time".equals(normalizedScope)) {
            String sql = "SELECT tl.started_at, tl.category, tl.duration_minutes, COALESCE(tl.note,'') " +
                    "FROM record_tag_link rtl " +
                    "JOIN time_log tl ON tl.id = rtl.record_id AND rtl.record_type = 'time_log' " +
                    "JOIN tag tg ON tg.id = rtl.tag_id " +
        ...
        if ("expense".equals(normalizedScope)) {
            String sql = "SELECT e.occurred_on, e.category, e.amount_cents, COALESCE(e.note,'') " +
                    "FROM record_tag_link rtl " +
                    "JOIN expense e ON e.id = rtl.record_id AND rtl.record_type = 'expense' " +
                    "JOIN tag tg ON tg.id = rtl.tag_id " +
    ...
    private ReviewReport buildReport(String periodName, String startTimeString, String endTimeString,
        ...
        String projSql = "SELECT * FROM (" +
                "SELECT p.id, p.name, " +
                "  COALESCE((SELECT SUM(t.duration_minutes) FROM time_log t JOIN record_project_link rpl ON rpl.record_type = 'time_log' AND rpl.record_id = t.id WHERE rpl.project_id = p.id AND t.started_at >= ? AND t.started_at < ? AND t.is_deleted = 0), 0) as pt, "
                +
                "  COALESCE((SELECT SUM(i.amount_cents) FROM income i JOIN record_project_link rpl ON rpl.record_type = 'income' AND rpl.record_id = i.id WHERE rpl.project_id = p.id AND i.occurred_on >= ? AND i.occurred_on <= ? AND i.is_deleted = 0), 0) as pi, "
                +
                "  COALESCE((SELECT SUM(e.amount_cents) FROM expense e JOIN record_project_link rpl ON rpl.record_type = 'expense' AND rpl.record_id = e.id WHERE rpl.project_id = p.id AND e.occurred_on >= ? AND e.occurred_on <= ? AND e.is_deleted = 0), 0) as pe "
                +
